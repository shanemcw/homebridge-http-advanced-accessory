...
Target-value changes and individual commands need different debounce choices. A positive `setterDelay` combines writes to the same characteristic and sends only the last one; this can suit a slider, but would lose deliberate repeated volume-up presses. Set the device's `setterDelay` to `0` to preserve those presses, including when a positive shared default exists. This setting currently applies to every setter on that device. Request spacing is separate from debounce: spacing delays admission without deliberately replacing an earlier write, while queue limits and deadlines can still reject work. Disabling debounce alone does not promise serial completion when concurrency allows overlapping requests.

A setter without a getter can send HTTP commands without creating a cached observation or confirmation window. HomeKit may retain a written characteristic value, but that value is not independent evidence of the device's actual state. Use genuine feedback when available and choose a HomeKit characteristic appropriate to a command or a readable value. HTTP method does not determine these semantics: a legacy GET request can send a command, and a POST request can read state. A per-action command mode is planned for review; no such configuration option exists in this version.
